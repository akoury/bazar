<template>
    <div class="flex flex-wrap rounded-lg bg-white shadow-md sm:h-full">
        <div class="w-full sm:w-3/5 h-64 sm:h-auto overflow-hidden p-2 sm:p-4 border-b sm:border-r border-gray-300 flex flex-col">
        ...
        <div class="sm:w-2/5 p-6">
            ...
            <svg height="25" width="23" class="mb-4 text-yellow-500 fill-current inline">
                <polygon points="9.9, 1.1, 3.3, 21.78, 19.8, 8.58, 0, 8.58, 16.5, 21.78"/>
            </svg>
            <svg height="25" width="23" class="mb-4 text-gray-500 fill-current inline">
                <polygon points="9.9, 1.1, 3.3, 21.78, 19.8, 8.58, 0, 8.58, 16.5, 21.78"/>
            </svg>
            <h2 class="text-red-600 text-xl mb-1 font-light line-through">$ {{ price }}</h2>
            <h2 class="text-teal-500 text-4xl font-light mb-6">$ {{ price }}</h2>
            <p class="text-gray-700 text-xl font-light leading-normal mb-6">{{ model.description }}</p>

            <fieldset v-for="attribute in attributes" :key="attribute.id">
                <legend class="block uppercase tracking-wider text-teal-300 text-sm font-bold mb-2">
                    {{ attribute.name }}
                </legend>
                ...
                <div class="inline-flex mb-6 flex-wrap">
                    <div v-for="value in attribute.values" :key="value.id" class="mr-2 flex flex-col items-center">
                        <label class="bg-gray-300 hover:bg-gray-300 text-gray-700 p-3 w-full cursor-pointer rounded border border-dashed border-transparent mb-2" :class="available(value)">
            ...
            <div v-if="selectedProduct.item_quantity > 0 && selectedProduct.deleted_at == null">
                <form @submit.prevent="order">
                    <label for="quantity" class="uppercase tracking-wider text-teal-300 text-sm font-bold mb-2">
                        Quantity
                    </label>
                    <input id="quantity" type="number" name="quantity" v-model="quantity" min="1" class="appearance-none w-full bg-gray-300 text-gray-700 border border-gray-300 rounded py-3 px-4 mt-2 mb-6" required>
                    <p class="text-gray-700 text-xl font-light mb-6">Delivered in 4 days</p>
                    <button type="submit" :disabled="processing" :class="{'cursor-not-allowed': processing }" class="bg-teal-500 hover:bg-teal-600 text-white py-4 px-4 w-full rounded mb-4">Order {{ quantity }} for {{ totalPriceInDollars }} $</button>
                </form>
                <button @click="addToCart" :disabled="processing" :class="{'cursor-not-allowed': processing }" class="bg-blue-500 hover:bg-blue-600 text-white py-4 px-4 w-full rounded">Add {{ quantity }} to Cart</button>
            </div>
            <div v-else>
                <p class="text-red-500 text-center font-normal mt-6 p-3 bg-red-100 rounded">
    ...
    methods: {
        ...
        available(value) {
            ...
            return {
                ...
                'border-red-500': !isAvailable && !this.combinations.find(combination => JSON.stringify(combination.values) === JSON.stringify(similarValues) && combination.available)
            }
        },
        productById(id) {
